...
export async function POST(req: Request) {
  try {
    ...
    const followUpSteps = "Maintain hydration, complete the entire prescribed therapeutic course, and return for clinical follow-up if symptoms persist.";
    ...
    if (!smtpPass) {
      emailMessage = "SMTP_PASS environment variable is missing on Vercel. Please add your 16-character Gmail App Password in Vercel Settings -> Environment Variables.";
      console.warn(emailMessage);
    } else if (!targetEmail || targetEmail.includes("example.com")) {
      emailMessage = `Cannot send email to invalid/demo address: ${targetEmail}`;
    } else {
      try {
        const transporter = nodemailer.createTransport({
          ...
          auth: {
            user: smtpUser,
            pass: smtpPass.replace(/\s+/g, ""), // strip accidental spaces
          },
        });
        ...
        const htmlContent = `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 24px; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #ffffff; color: #1e293b;">
            ...
            <div style="background-color: #f8fafc; padding: 14px; border-radius: 10px; margin-bottom: 20px; font-size: 13px;">
              ...
              <p style="margin: 2px 0;"><strong>Consultation Date:</strong> ${new Date().toLocaleDateString()}</p>
            </div>
        ...
        emailSent = true;
        emailMessage = `Prescription & care plan email successfully delivered to ${targetEmail}`;
      } catch (mailErr: any) {
        console.error("Nodemailer error:", mailErr);
        emailMessage = `Mail delivery failed: ${mailErr.message}`;
      }
    }
