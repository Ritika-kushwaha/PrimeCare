...
# Replace handleRejectDoctor with credential deletion + rejection email
'use client';
...
export default function AdminDashboardPage() {
 ...
 -replace "const handleRejectDoctor = async \(appId: string\) => \{[\s\S]*?loadData\(\);\s*\};", @"
  const handleRejectDoctor = async (app: DoctorApplication) => {
    try {
      const cleanAppEmail = app.email.trim().toLowerCase();

      // 1. Mark rejected and wipe credentials from pc_users
      await fetch('/api/sync/applications', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      // 2. Dispatch Rejection Notice Email to applicant
      try {
        await fetch('/api/notifications/email', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      await loadData();
      setActionSuccessMsg('Application for ' + app.name + ' rejected. Credentials removed and notice email dispatched.');
      setTimeout(() => setActionSuccessMsg(''), 5000);
    } catch {
